...
var allMessageText string

func makeRequestAndGetData(input string, params structs.Params, extraOptions structs.ExtraOptions) string {
	checkInputLength(input, extraOptions.DisableInputLimit)

	resp, err := providers.NewRequest(translator.Translate(input, "uz", "en"), params, extraOptions)
	...
	for scanner.Scan() {
		...
		previousText = newText
		fullText += mainText
		allMessageText += mainText + "\n" // Append to allMessageText

		if !extraOptions.IsGetWhole {
			fmt.Print("Main text " + mainText)
		}
	}
	...
	// Translate the collected messages and display
	if allMessageText != "" {
		translatedText := translator.Translate(allMessageText, "en", "uz")
